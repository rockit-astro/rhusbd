...
all:
	...
	${RPMBUILD} --define "_version %(cat VERSION)" -ba rockit-rhusb.spec
	${RPMBUILD} --define "_version %(cat VERSION)" -ba python3-rockit-rhusb.spec
...
install:
	...
	@sudo pip3 install rockit.rhusb-$$(cat VERSION)-py3-none-any.whl
	@rm VERSION
	@cp rhusbd rhusb /bin/
